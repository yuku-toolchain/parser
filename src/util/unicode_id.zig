...
pub const canStartId = canStartIdentifierUnicode;
pub const canContinueId = canContinueIdentifierUnicode;
...
test "unicode_id can id" {
    ...
    // specific edge cases with UTF-8 sequences
    // '_' (underscore) and '\u{200D}' (zero-width joiner)
    const codepoint_slice = "_‍"; // Contains underscore and zero-width joiner
    const utf8view = try std.unicode.Utf8View.init(codepoint_slice);
    var iter = utf8view.iterator();

    // underscore should be valid as start character
    try t.expect(canStartId(iter.nextCodepoint() orelse unreachable));
    // zero-width joiner should be valid as continue character
    try t.expect(canContinueId(iter.nextCodepoint() orelse unreachable));

    // all ID_Continue codepoints
    for (0..std.math.maxInt(u21)) |ch| {
